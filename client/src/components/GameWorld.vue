<template>
  <TresCanvas class="game-world" clear-color="#87CEEB">
    ...
    <PlayerCharacter
      ...
      :character-data="gameStore.currentPlayer || undefined"
    />
...
const cameraPosition = computed<Position3D>(() => {
  ...
  return [
    playerPos[0] + cameraOffset[0],
    playerPos[1] + cameraOffset[1],
    playerPos[2] + cameraOffset[2],
  ] as Position3D;
});

// Player position from store
const playerPosition = computed<Position3D>(
  () => gameStore.playerPosition as Position3D
);
