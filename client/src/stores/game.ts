...
export const useGameStore = defineStore("game", () => {
  ...
  const playerPosition = computed(
    () => currentPlayer.value?.position || [0, 0, 0]
  );
    ...
    try {
      ...
      if (mapName === "tutorial_island") {
        ...
        const response = await fetch(
          `http://localhost:8000/api/maps/${mapName}`
        );
  ...
  async function loadTerrainModifications(mapName: string) {
    try {
      const response = await fetch(
        `http://localhost:8000/api/terrain-modifications/${mapName}`
      );
  ...
  function addSpellEffect(effect: SpellEffect) {
    ...
    setTimeout(() => {
      const index = activeSpellEffects.value.findIndex(
        (e) => e.id === effect.id
      );
      if (index >= 0) {
